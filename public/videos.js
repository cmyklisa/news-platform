...
const CHANNEL_INFO = [
  ...
  { match: /公視|PTSNEWS|ptsnews/i,    name: '公視新聞',  color: '#0f6b8b' },
  { match: /民視|FTV|FTVNEWS/i,        name: '民視新聞',  color: '#c41e3a' },
  { match: /三立|SETN|SET News|setn/i, name: '三立新聞',  color: '#0d9a3a' },
  { match: /中央社|CNA/i,              name: '中央社',    color: '#5a4f40' },
...
const DEFAULT_CHANNEL = { name: 'YouTube', color: '#cc0000' };
...
function detectChannel(title) {
  for (const ch of CHANNEL_INFO) if (ch.match.test(title)) return ch;
...
function render(items) {
  ...
  vidGrid.innerHTML = items.map(it => {
    ...
    return `
      <a class="vid-row" href="${escapeHTML(it.url)}" target="_blank" rel="noopener"
         style="--ch-color:${ch.color}">
        <span class="vid-row-channel">${escapeHTML(ch.name)}</span>
        <span class="vid-row-title">${highlight(cleaned)}</span>
        <span class="vid-row-time">${timeAgo(it.published_at)}</span>
        <span class="vid-row-ext" aria-hidden="true">↗</span>
      </a>
    `;
  }).join('');
}
...
async function load(q = '') {
  ...
  try {
    const url = q ? `/api/videos?q=${encodeURIComponent(q)}&limit=200` : `/api/videos?limit=200`;
    const r = await fetch(url, { signal: searchAbort.signal });
    const { items = [] } = await r.json();
    vidMeta.textContent = q
      ? `搜尋「${q}」· ${items.length} 部`
      : `${items.length} 部影音 · 點任一標題在 YouTube 觀看`;
    render(items);
    statusEl.textContent = '';
  } catch (err) {
    if (err.name !== 'AbortError') {
      statusEl.textContent = '載入失敗';
      vidGrid.innerHTML = `<div class="empty">無法載入</div>`;
    }
  }
}
